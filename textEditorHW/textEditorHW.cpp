...
public:
	...
	linked_list() {
		...
		while (choice != 0) {
			...
			cout << "2. Delete line\n3. Move line\n4. Replace line" << endl;
			cout << "5. Print\n6. Save\n7. Undo\n8. Open\n9. Print the next line\n10. Print the previous line" << endl;
			cin >> choice;
			cout << endl;
			...
			else if (choice == 4)
			{
				int lineGiven;
				string dataGiven;
				cout << "Enter line to change the text: ";
				cin >> lineGiven;
	...
	void whateverAddToTail(string dataGiven) {		//an extra function used to add to tail
		if (head == NULL)					//no node, empty linked list
		{
			node* temp;
			temp = new node;
	...
	void insertTextInBetween(string dataGiven, int lineGiven) {		//this function will insert text in the given line, and will push all the other lines
		if (lineGiven == 0)
		{
			cout << "No such line, line starts with 1" << endl;
		}
		else if (lineGiven == 1)
		{
			if (head == NULL)					//no node, empty linked list
	...
	void deleteLine(int lineGiven) {							//this function should delete anything in the given line, also decreases the numOfLines
		...
		{
			cout << "Number of lines are smaller than you entered" << endl;
		}
		else if (lineGiven < numOfLines)
		{
			undoCmd deletedLine;
	...
	void moveNtoM(int nLineGiven, int mLineGiven) {		//function used to Move line from X into line X2
		if (nLineGiven == 1)
		{
